...
namespace numcpp
{
    ...
    inline void matrix_dot(const Md &m1, const Md &m2, Md &outm)
    {
        if (m1.m[0].v.size() && m2.m.size())
            ...
            {
                LOG("Shape mismatch: "
                    << "matrix1 columns: " << m1.m[0].v.size() << ", "
                    << "matrix2 rows: " << m2.m.size());
                throw std::exception();
            }

        unsigned int m1x = 0;
        unsigned int m1y = 0;
        unsigned int m2y = 0; // m2y = m1x
    ...
    inline void matrix_add(Md &mat, const Vd &vec)
    {
        ...
        {
            LOG("Shape mismatch: "
                << "matrix columns: " << mat.m[0].v.size() << ", "
                << "vector size: " << vec.v.size());
            throw std::exception();
        }
...
namespace neural_net
{
    ...
    LayerDense::LayerDense(const int nInputs, const int nNeurons, double bias)
    {
        ...
        LOG("(dense layer init) initialized with matrix: " << std::endl
                                                           << weights);
    ...
    void LayerDense::Forward(const numcpp::Md &inputs)
    {
        ...
        LOG(" (dense layer forward) inputs * weights in forward method of dense layer. output: " << std::endl
                                                                                                 << output);
        numcpp::matrix_add(output, biases);
        LOG("(dense layer forward) biases added to output matrix from dot product of weights and input: " << std::endl
                                                                                                          << output);
    }
...
namespace neural_net
{
    ...
    void ActivationReLU::Forward(const numcpp::Md &input)
    {
        ...
        LOG("(ReLU forward) forward pass. matrix capped at min value of 0. output: " << std::endl
                                                                                     << output);
    }
    ...
    void ActivationSoftmax::Forward(const numcpp::Md &input)
    {
        ...
        LOG("(softmax forward) input matrix's highest value negated from all the elements in matrix. output: " << std::endl
                                                                                                               << negated);
        numcpp::Md exp = numcpp::matrix_exp(negated);
        LOG("(softmax forward) all the elements in the negated matrix exponentiated. output: " << std::endl
                                                                                               << exp);
        output = numcpp::matrix_normalize_in_axis(exp, 1);
        LOG("(softmax forward) exponentiated matrix is normalized. output: " << std::endl
                                                                             << output);
    }
